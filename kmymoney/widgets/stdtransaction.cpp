...
StdTransaction::StdTransaction() :
  Transaction(*new StdTransactionPrivate, nullptr, MyMoneyTransaction(), MyMoneySplit(), 0)
{
}
...
StdTransaction::StdTransaction(const StdTransaction& other) :
  Transaction(*new StdTransactionPrivate(*other.d_func()))
{
}
...
KMyMoneyRegister::Action StdTransaction::actionType() const
{
  ...
  for (it_s = d->m_transaction.splits().begin(); it_s != d->m_transaction.splits().end(); ++it_s) {
    ...
    if (acc.accountGroup() == eMyMoney::Account::Income
      ...
      action = d->m_split.shares().isNegative() ? ActionWithdrawal : ActionDeposit;
      break;
    }
  }
...
void StdTransaction::arrangeWidgetsInForm(QMap<QString, QWidget*>& editWidgets)
{
  ...
  arrangeWidget(d->m_form, 0, LabelColumn1, editWidgets["account-label"]);
  arrangeWidget(d->m_form, 0, ValueColumn1, editWidgets["account"]);
  arrangeWidget(d->m_form, 1, LabelColumn1, editWidgets["cashflow"]);
  arrangeWidget(d->m_form, 1, ValueColumn1, editWidgets["payee"]);
  arrangeWidget(d->m_form, 2, LabelColumn1, editWidgets["category-label"]);
  arrangeWidget(d->m_form, 2, ValueColumn1, editWidgets["category"]->parentWidget());
  arrangeWidget(d->m_form, 3, LabelColumn1, editWidgets["tag-label"]);
  arrangeWidget(d->m_form, 3, ValueColumn1, editWidgets["tag"]);
  arrangeWidget(d->m_form, 4, LabelColumn1, editWidgets["memo-label"]);
  arrangeWidget(d->m_form, 4, ValueColumn1, editWidgets["memo"]);
  if (haveNumberField()) {
    arrangeWidget(d->m_form, 1, LabelColumn2, editWidgets["number-label"]);
    arrangeWidget(d->m_form, 1, ValueColumn2, editWidgets["number"]);
  }
  arrangeWidget(d->m_form, 2, LabelColumn2, editWidgets["date-label"]);
  arrangeWidget(d->m_form, 2, ValueColumn2, editWidgets["postdate"]);
  arrangeWidget(d->m_form, 3, LabelColumn2, editWidgets["amount-label"]);
  arrangeWidget(d->m_form, 3, ValueColumn2, editWidgets["amount"]);
  arrangeWidget(d->m_form, 5, LabelColumn2, editWidgets["status-label"]);
  arrangeWidget(d->m_form, 5, ValueColumn2, editWidgets["status"]);
...
void StdTransaction::tabOrderInForm(QWidgetList& tabOrderWidgets) const
{
  ...
  while (it_s != taborder.constEnd()) {
    if (*it_s == "account") {
      ...
      tabOrderWidgets.append(focusWidget(d->m_form->cellWidget(1, LabelColumn1)));
    } else if (*it_s == "payee") {
      ...
      QWidget* w = d->m_form->cellWidget(2, ValueColumn1);
      tabOrderWidgets.append(focusWidget(w));
      w = w->findChild<QPushButton*>("splitButton");
      if (w)
        tabOrderWidgets.append(w);
    } else if (*it_s == "tag") {
      tabOrderWidgets.append(focusWidget(d->m_form->cellWidget(3, ValueColumn1)));
    } else if (*it_s == "memo") {
      tabOrderWidgets.append(focusWidget(d->m_form->cellWidget(4, ValueColumn1)));
    } else if (*it_s == "number") {
      if (haveNumberField()) {
        if ((w = focusWidget(d->m_form->cellWidget(1, ValueColumn2))))
          tabOrderWidgets.append(w);
      }
    } else if (*it_s == "date") {
      ...
      tabOrderWidgets.append(focusWidget(d->m_form->cellWidget(5, ValueColumn2)));
    }
    ++it_s;
  }
}
...
void StdTransaction::arrangeWidgetsInRegister(QMap<QString, QWidget*>& editWidgets)
{
  ...
  if (haveNumberField())
    arrangeWidget(d->m_parent, d->m_startRow + 0, NumberColumn, editWidgets["number"]);
  ...
  arrangeWidget(d->m_parent, d->m_startRow + 0, DetailColumn, editWidgets["payee"]);
  arrangeWidget(d->m_parent, d->m_startRow + 1, DetailColumn, editWidgets["category"]->parentWidget());
  arrangeWidget(d->m_parent, d->m_startRow + 2, DetailColumn, editWidgets["tag"]);
  arrangeWidget(d->m_parent, d->m_startRow + 3, DetailColumn, editWidgets["memo"]);
  arrangeWidget(d->m_parent, d->m_startRow + 0, PaymentColumn, editWidgets["payment"]);
  arrangeWidget(d->m_parent, d->m_startRow + 0, DepositColumn, editWidgets["deposit"]);
...
void StdTransaction::tabOrderInRegister(QWidgetList& tabOrderWidgets) const
{
  ...
  while (it_s != taborder.constEnd()) {
    if (*it_s == "number") {
      if (haveNumberField()) {
        if ((w = focusWidget(d->m_parent->cellWidget(d->m_startRow + 0, NumberColumn))))
          tabOrderWidgets.append(w);
      }
    } else if (*it_s == "date") {
      ...
      // it's one of our own widgets, so we actually don't care. Just make sure, that we don't
      // go haywire when someone changes the KMyMoneyCategory object ...
      w = d->m_parent->cellWidget(d->m_startRow + 1, DetailColumn);
      tabOrderWidgets.append(focusWidget(w));
      w = w->findChild<QPushButton*>("splitButton");
      if (w)
        tabOrderWidgets.append(w);
    } else if (*it_s == "tag") {
      tabOrderWidgets.append(focusWidget(d->m_parent->cellWidget(d->m_startRow + 2, DetailColumn)));
    } else if (*it_s == "memo") {
      tabOrderWidgets.append(focusWidget(d->m_parent->cellWidget(d->m_startRow + 3, DetailColumn)));
    } else if (*it_s == "payment") {
      tabOrderWidgets.append(focusWidget(d->m_parent->cellWidget(d->m_startRow + 0, PaymentColumn)));
    } else if (*it_s == "deposit") {
      tabOrderWidgets.append(focusWidget(d->m_parent->cellWidget(d->m_startRow + 0, DepositColumn)));
    } else if (*it_s == "state") {
