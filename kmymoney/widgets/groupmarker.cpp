...
const QString& GroupMarker::text() const
{
...
void GroupMarker::paintRegisterCell(QPainter *painter, QStyleOptionViewItem &option, const QModelIndex &index)
{
  ...
  if (d->m_showDate) {
    font.setBold(false);
    cellRect.setX(d->m_parent->horizontalHeader()->sectionPosition(DateColumn));
    cellRect.setWidth(d->m_parent->horizontalHeader()->sectionSize(DateColumn));
    painter->setFont(font);
