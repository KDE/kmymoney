...
KMMApplication::KMMApplication(int &argc, char **argv, KMyMoney *kmymoney) : QApplication(argc, argv){
...
//  }
  ...
  if (isNoCatchOption) {
    ...
    rc = run(std::move(splash), url, isNoFileOption);
  } else {
    try {
      rc = run(std::move(splash), url, isNoFileOption);
...
bool KMMApplication::event(QEvent * event){
  switch (event->type()) {
    case QEvent::FileOpen:
      raise(SIGSTOP);
      auto fileOpenEvent = dynamic_cast<QFileOpenEvent *>(event);
      std::unique_ptr<QSplashScreen> splash(nullptr);
      openFile(splash, fileOpenEvent->url(), false);
      return true;
  }
  return QApplication::event(event);
}

int KMMApplication::run(std::unique_ptr<QSplashScreen> splash, const QUrl & file, bool noFile)
{
  ...
  const int rc = this->exec();      //krazy:exclude=crashy
  return rc;
}
void KMMApplication::openFile(std::unique_ptr<QSplashScreen> &splash, const QUrl &file, bool noFile) const
{
  QString importfile;
  QUrl url;
