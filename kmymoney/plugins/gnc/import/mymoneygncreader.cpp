...
GncObject::GncObject()
    : pMain(nullptr)
    , m_subElementList(nullptr)
    , m_subElementListCount(0)
    , m_dataElementList(nullptr)
    , m_dataElementListCount(0)
    , m_dataPtr(nullptr)
    , m_state(0)
    , m_anonClassList(nullptr)
    , m_anonClass(0)
{
}
...
#define MAXFILEHIDEF 99.99
    ...
    while (m_fileHideFactor == 0.0) {
        m_fileHideFactor = QInputDialog::getDouble(nullptr,
...
void MyMoneyGncReader::convertPrice(const GncPrice *gpr)
{
    ...
    if (gpr->commodity()->isCurrency()) {
        ...
        if (gncdebug)
            qDebug() << "Searching map, key = " << gpr->commodity()->id() << ", found id =" << e.id();
...
void MyMoneyGncReader::convertTemplateSplit(const QString& schedName, const GncTemplateSplit *gsp)
{
    ...
    for (i = 0; i < gsp->kvpCount(); i++) {
        ...
        if ((slot.key() == "sched-xaction") && (slot.type() == "frame")) {
            ...
            KMyMoneyMoneyValidator v(nullptr);
...
for (map_accountIds::const_iterator it = m_mapIds.cbegin(); it != m_mapIds.cend(); ++it) {
    if (gncdebug)
        ...
        for (it = m_currencyCount.cbegin(); it != m_currencyCount.cend(); ++it) {
        ...
        if ((m_orCount = m_messageList["OR"].count()) > 0)
            sectionsToReport.append("OR");
        if ((m_scCount = m_messageList["SC"].count()) > 0)
            sectionsToReport.append("SC");
        ...
        for (si = 0; (si < sectionsToReport.count()) && !exit; ++si) {
            ...
            switch (KMessageBox::questionTwoActionsCancel(nullptr, buildReportSection(sectionsToReport[si]), PACKAGE, yesItem, noItem)) {
        ...
        for (si = 0; si < m_suspectList.count(); ++si) {
            ...
            KMessageBox::information(nullptr, i18n("Problems were encountered in converting schedule '%1'.", sc.name()), PACKAGE);
...
void MyMoneyGncReader::checkInvestmentOption(QString stockId)
{
    ...
    if (m_investmentOption == 0) {
        ...
        if (singleInvAccId.isEmpty()) { // if the account has not yet been created
            QString invAccName;
            while (!ok) {
                invAccName = QInputDialog::getText(nullptr,
...
#endif
                    switch (KMessageBox::questionTwoActions(nullptr,
                                                            i18n("%1 is not an Investment Account. Do you wish to make it one?", invAcc.name()),
                                                            PACKAGE,
                                                            KMMYesNo::yes(),
                                                            KMMYesNo::no())) {
...
void MyMoneyGncReader::getPriceSource(MyMoneySecurity stock, QString gncSource)
{
    ...
    QMap<QString, QString>::const_iterator it;
    for (it = m_mapSources.cbegin(); it != m_mapSources.cend(); ++it) {
        if (it.key() == gncSource) {
