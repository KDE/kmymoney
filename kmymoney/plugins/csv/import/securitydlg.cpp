    ...
    : ui(new Ui::SecurityDlg)
{
    ...
    connect(ui->cbSecurity, &QComboBox::currentIndexChanged, this, &SecurityDlg::slotIndexChanged);
...
{
    if (ui->leNewName->text().isEmpty() && ui->leNewSymbol->text().isEmpty() && ui->cbSecurity->currentIndex() == -1) {
