/*
    ...
    SPDX-FileCopyrightText: 2000-2003 Ace Jones <acejones@users.sourceforge.net>
...
const QString MyMoneyQifReader::processAccountEntry(bool resetAccountId)
{
    ...
    account.setName(extractLine('N'));
    qDebug() << "Process account" << account.name();
    ...
    if (resetAccountId) {
        ...
        d->finishStatement();
        m_account = account;
        d->st.m_accountId = m_account.id();  // needed here to make sure account selection dialog is shown...
        d->st.m_strAccountName = m_account.name(); // ...and that it displays the name of the account as imported from QIF
        d->st.m_strAccountNumber = m_account.name(); // ...and that it remembers that name as a StatementKey to match next time
