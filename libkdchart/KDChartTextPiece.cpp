    ...
    :QObject(0)
{
    ...
    if( QStyleSheet::mightBeRichText( text ) ) {
        _isRichText = true;
        _richText = new QSimpleRichText( text, font );
        //qDebug( "richtext width %s", QString::number(_richText->width()).toLatin1());
	//qDebug( "richtext height %s", QString::number(_richText->height()).toLatin1());
        _richText->adjustSize();
        //qDebug( "richtext width %s", QString::number(_richText->width()).toLatin1());
	//qDebug( "richtext height %s", QString::number(_richText->height()).toLatin1());
    ...
    if( _dirtyMetrics ) {
        _metrics = new QFontMetrics( font );
        //qDebug("dirty metrics text: %s", text.toLatin1());
    }
    else{
        p->save();
        p->setFont( font );
        _metrics = new QFontMetrics( p->fontMetrics() );
        //qDebug ( "drawing metrics text: %s", text.toLatin1() );
...
int KDChartTextPiece::height() const
{
  ...
  if( _isRichText ) {
    //qDebug ("_richText height %s", QString::number(_richText->height()).toLatin1());
        return _richText->height();
  }
  else {

    //qDebug ("_metrics height %s", QString::number(_metrics->height()).toLatin1());
