    ...
    if( _fontScaleGlobal && 0 == w && 0 == h ){
      //Now calculate the size of the box based upon the content!
        ...
        if ( _fontSize ) {
            ...
            : _fontSize );
	    //qDebug("\n_fontSize * qMin(areaWidthP1000, areaHeightP1000)    %i * qMin(%f, %f) text: %s", _fontSize , areaWidthP1000, areaHeightP1000, content().text().toLatin1());
        }
...
//        }
    ...
    getTrueShift( areaWidthP1000, areaHeightP1000, h,
                  dX, dY );
    //qDebug("trueRect:  x %i  y %i    w %i  h %i text: %s", anchor.x()+x+dX, anchor.y()+y+dY, w,h, content().text().toLatin1());
